...
async function dormir (player, fugiu, lobo, loboVive, pausa, dormiuNaCaverna){
    ...
    if (fugiu == true) {
        ...
        switch (parseInt(escolha)) {
            case 1:
                if (loboVive == false) {
                    await exibirTexto(`"Engraçado você dizer isso, eu não te vi ficar com medo quando você teve que lutar contra aquele lobo, na verdade`, 50)
                    ...
                    await exibirTexto(`A partir de agora o seu nome é ${player.nome}. Fui eu mesmo quem escolheu, já que você `, 50)
        ...
        if (loboVive == false) {
            ...
            if (player.itens == 0) {
                ...
                switch (parseInt(escolha)) {
                    ...
                    case 2:
                        await exibirTexto("Bem, eu não faria isso se eu fosse você", 50)
                        await exibirTexto("...", 600)
                        await exibirTexto(" Mas como eu não sou você e as ", 50)
            ...
            await player.status()
            await lobo.status()
            await exibirTexto("Nos vemos num futuro próximo! (ou não).", 50)
            pausa("Precione enter para acordar")
            await cenaFinal(player, lobo, loboVive, pausa, dormiuNaCaverna)
