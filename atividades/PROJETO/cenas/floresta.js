...
async function acordar(fugiu, player, pausa){
    ...
    if (fugiu) {
        ...
        await exibirTexto("Você acorda perto de uma clareira, o lugar ao seu redor parece ser um campo aberto cercado por árvores altas e densas, A luz da lua penetra suavemente pelas copas das árvores, criando padrões de sombra no chão. Você se lembra da mulher misteriosa e suas palavras ainda ecoam na sua mente.", 65);
        escolher()
    }
        ...
        switch (parseInt(escolha)) {
            ...
            case 2:
                if (player.itens > 0){
                    ...
                    await exibirTexto("Você estende a barrinha de cereal em direção ao lobo. O animal observa com cautela, seus olhos fixos na comida. O rosnado do lobo diminui gradualmente, e ele parece intrigado pela oferta.", 65)
            ...
            case 3:
                ...
                await exibirTexto("Com a visão panorâmica da floresta ao seu redor, você se sente mais seguro.Você se encosta na arvore e, apesar de não ser o lugar mais confortável do mundo, eventualmente você pega no sono com o som dos grilos e da chuva caindo nas folhas.", 65)
    ...
    async function lutar(player, inimigo, textoVenceu, textoPerdeu){
        ...
        if (player.vida < 1){
            console.clear()
            await exibirTexto(textoPerdeu, 65)
            ...
            console.clear()
            await exibirTexto(textoVenceu, 65)
            await pausa()
